// =============================================================================
// PCBTest_04_SDBoot_BarrelLED_Audio_Motor
//
// Minimal bring-up test - NOT part of the main Sentinel Beam firmware.
// Builds on PCBTest_03: same SD-to-PSRAM load, LED mirroring, and one-shot
// fire.wav playback, but now also spins the barrel motor for as long as the
// audio plays - so this test additionally exercises the TMC2209/stepper
// wiring, and lets you visually+audibly confirm motor and audio are
...
//
// Speed note: the main firmware's reference point is 7467 steps/sec = ~70
// RPM at the barrel (4:1 gear reduction already baked into that number).
// Scaling linearly for ~40 RPM: 7467 * (40/70) = 4267 steps/sec. Acceleration
// reuses the main firmware's value as-is (this test doesn't need a specific
// spin-up time, just "spin up").
//
// Sequence (onboard LED + barrel strip always match):
//   Boot                           -> RED
//   Reading fire.wav from SD       -> YELLOW
//   Loaded into PSRAM, SD happy    -> GREEN
//   fire.wav + motor spin-up start -> (stays GREEN while both run)
//   Playback finished              -> motor commanded to stop, LEDs BLUE
//   Any failure along the way      -> RED (stays), reason printed to Serial
//
// This is a one-shot test: loop() feeds the audio decoder and, once playback
// ends, tells the motor to decelerate to a stop. Nothing repeats afterward.
// =============================================================================
...
void setup() {
    ...
    Serial.println();
    Serial.println("[PCBTest] SD -> PSRAM -> playback + motor bring-up test starting...");
    ...
    // Motor and audio start together.
    stepper->runForward();
    audioPlaying = true;

    Serial.println("[PCBTest] Playing fire.wav and spinning motor at ~40 RPM - LEDs staying GREEN until playback finishes");
}
...
void loop() {
    ...
    if (!stillRunning) {
        // Playback reached EOF (or hit an error) - stop the motor and mark done.
        audioWav->stop();
        audioPlaying = false;
        ...
        Serial.println("[PCBTest] Playback finished, motor commanded to stop - LEDs BLUE");
